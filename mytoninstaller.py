...
def DownloadDump():
	...
	dumpSize = requests.get(url + "/dumps/latest_testnet.tar.size.archive.txt").text
	...
	# download dump
	cmd = "curl -Ls {url}/dumps/latest_testnet.tar.lz | pv | plzip -d -n8 | tar -xC /var/ton-work/db".format(url=url)
	os.system(cmd)
#end define
